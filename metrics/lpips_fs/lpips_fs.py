...
class LPIPS(nn.Module):
    ...
    def forward(self, data, retPerLayer=False, normalize=False):
        with torch.no_grad():
            data = 2 * data - 1
            all_feats = []
            data = self.scaling_layer(data) if self.version=='0.1' else data
            outs = self.net.forward(data)
            n_samples = data.shape[0]
            for i in range(n_samples):
                feats = {}
                for kk in range(self.L):
                    feats[kk] = lpips.normalize_tensor(outs[kk][i:i+1])
            ...
            for i in range(n_samples):
                for j in range(i + 1, n_samples):
                    ...
                    if(self.lpips):
                        if(self.spatial):
                            res = [upsample(self.lins[kk](diffs[kk]), out_HW=data.shape[2:]) for kk in range(self.L)]
                        else:
                            res = [spatial_average(self.lins[kk](diffs[kk]), keepdim=True) for kk in range(self.L)]
                    else:
                        if(self.spatial):
                            res = [upsample(diffs[kk].sum(dim=1,keepdim=True), out_HW=data.shape[2:]) for kk in range(self.L)]
                        else:
                            res = [spatial_average(diffs[kk].sum(dim=1,keepdim=True), keepdim=True) for kk in range(self.L)]
                    val = 0
                    for l in range(self.L):
                        val += res[l]
                    all_vals.append(val.detach().cpu().numpy())
